...
class Dots:
    ...
    def eat(self, pacman_x, pacman_y):
        def filter_dots(dot_list):
            return [dot for dot in dot_list
                    if dist(dot.x, dot.y, pacman_x, pacman_y) > self.EAT_DIST]

        self.top_row = filter_dots(self.top_row)
        self.bottom_row = filter_dots(self.bottom_row)
        self.left_col = filter_dots(self.left_col)
        self.right_col = filter_dots(self.right_col)
